...
check_panel_api() {
  ...
  if _port_open 2086; then
    ...
    log_warning "  [panel] Panel HTTP port 2086 NOT open"
  fi

  if _port_open 2087; then
    log_success "  [panel] Panel HTTPS port 2087 open"
  else
    log_warning "  [panel] Panel HTTPS port 2087 NOT open"
  fi

  if _port_open 8090; then
    log_success "  [panel] OLS Admin port 8090 open"
  else
    log_warning "  [panel] OLS Admin port 8090 NOT open"
  fi
  ...
  if [[ "$panel_http" =~ ^(200|301|302|401|403)$ ]]; then
    ...
    log_warning "  [panel] panel endpoint returned HTTP $panel_http"
  fi
