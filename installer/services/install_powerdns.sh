...
CREATE USER IF NOT EXISTS '${PDNS_DB_USER}'@'::1' IDENTIFIED BY '${PDNS_DB_PASSWD}';
ALTER USER '${PDNS_DB_USER}'@'localhost' IDENTIFIED BY '${PDNS_DB_PASSWD}';
ALTER USER '${PDNS_DB_USER}'@'127.0.0.1' IDENTIFIED BY '${PDNS_DB_PASSWD}';
ALTER USER '${PDNS_DB_USER}'@'::1' IDENTIFIED BY '${PDNS_DB_PASSWD}';
GRANT ALL PRIVILEGES ON \`${PDNS_DB}\`.* TO '${PDNS_DB_USER}'@'localhost';
GRANT ALL PRIVILEGES ON \`${PDNS_DB}\`.* TO '${PDNS_DB_USER}'@'127.0.0.1';
GRANT ALL PRIVILEGES ON \`${PDNS_DB}\`.* TO '${PDNS_DB_USER}'@'::1';
FLUSH PRIVILEGES;
SQL
